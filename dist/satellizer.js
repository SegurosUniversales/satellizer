    ...
    .provider('$auth', ['SatellizerConfig', function(config) {
      ...
      this.$get = [
        ...
        function($q, shared, local, oauth) {
          ...
          $auth.userinfo = function(name) {
            return oauth.userinfo(name);
          };
    ...
    .factory('SatellizerOauth', [
      ...
      function($q, $http, config, utils, shared, Oauth1, Oauth2) {
        ...
        Oauth.authenticate = function(name, userData) {
          ...
          Oauth.userinfo = function(name) {

            var options = config.providers[name] ;
            ...
            return $http.get(options.userInfo);
        ...
        return function() {
          ...
          Oauth2.exchangeForToken = function(oauthData, userData) {
            ...
            var exchangeForTokenUrl = config.baseUrl ? utils.joinUrl(config.baseUrl, defaults.url) : defaults.url;
              var BasicAuthentication = btoa(defaults.clientId + ':' + defaults.secret);

              var postString = 'grant_type='+data['grant_type']+'&code='+data['code']+'&redirect_uri='+data['redirectUri'];
...
//            return $http.post(exchangeForTokenUrl, postString, { withCredentials: config.withCredentials });
              return $http(req);
